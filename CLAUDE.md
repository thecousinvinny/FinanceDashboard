...
- **200px sidebar** (`var(--color-bg-surface)`, `borderRight: 1px solid var(--color-grid-border)`): mini month navigator (`sidebarYear`/`sidebarMonth` state — syncs automatically as the user scrolls the main grid), per-type legend toggles (`hiddenTypes: Set<EventType>` local state — clicking hides that type from the grid only), Today button scrolls main grid to today, Add Calendar button → settings sheet.
...
- **Main grid** (`var(--color-bg-base)`): sticky DOW header + scrollable `<div ref={monthGridRef}>` containing a **single continuous week-based grid** — `notionWeeks.map(weekStart => ...)` with no month section breaks, no sticky month labels.
  - Each row: one week (`position: relative`, CSS Grid `repeat(7,1fr)`, `borderBottom: 1px solid var(--color-grid-border)`, `minHeight: CELL_MIN_H`). Module-level sizing constants are the single source of truth: `MAX_VIS_EVENTS = 4`, `EV_ROW_H = 20`, `DATE_ROW_H = 20`, `CELL_PAD_V = 9`, `OVERFLOW_H = 14`, `CELL_MIN_H = 123`.
