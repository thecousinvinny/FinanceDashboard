...
`NEXT_PUBLIC_SUPABASE_*` — required to boot. `GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET` — server-only; used by `src/app/api/calendar/route.ts` to refresh Google OAuth tokens for Calendar CRUD. `NEXT_PUBLIC_GOOGLE_MAPS_API_KEY` — optional; enables the Places Autocomplete location picker in calendar event sheets; location rows render without it but suggestions won't load. Auth is Google OAuth via Supabase — configure the Google provider in the Supabase dashboard and set the redirect URL to `{origin}/home`.
...
- `home/HomeHero.tsx` — the sparkline card on the Home tab. Props: `{ spent, points, annualPoints }`. Two views (month / year) on a **two-panel horizontal sliding rail**: outer div is `width: 200%`, inner contains both `<SparkChart>` instances side by side; `transform: translateX(0/-50%)` drives the slide. A dark left-to-right gradient overlay (`rgba(22,30,39,0.85) → transparent`) sits at `z-index: 1` so text stays readable over the chart. Text overlay at `z-index: 2` contains: eyebrow label + two pill-dots page indicator, compact legend row (updates live on hover), and the `SlotNumber` hero number. **Gesture coordination**: HomeHero owns a `gestureMode = useRef<'undecided' | 'swiping' | 'scrubbing'>('undecided')` ref that it shares with both `<SparkChart>` instances. Capture-phase handlers (`onTouchStartCapture/MoveCapture/EndCapture`) detect quick horizontal flicks (>10px, dx > dy×1.5) and set `gestureMode = 'swiping'` + call `e.stopPropagation()` to switch views; they bail out immediately if `gestureMode === 'scrubbing'` so SparkChart-owned long presses are never misread as swipes. `useEffect` clears `hoveredPoint` on view switch so the legend resets. `annualPoints` use month abbreviations (`'Jan'…'Dec'`) as the `day` field; the home page builds them by grouping monthly totals from Jan to the current month.
...
- `home/SparkChart.tsx` — area chart used by `HomeHero`. Three series: `inc` (income), `exp` (non-sub expenses), `sub` (subscription payments). Values are **individual daily amounts** (not cumulative). `DayPoint { day, label, exp, inc, sub }` — `day` is either a date number (`"1"`, `"31"`) for the monthly view or a month abbreviation (`"Jan"`) for the annual view. X-axis: up to 7 sparse absolute-positioned landmark labels; default `text-ink-faint`, white on hover, `text-ink-muted font-bold` for the last/rightmost label. Gradients use `gradientUnits="userSpaceOnUse"` with per-series `peakY` so opacity 0.95 falls exactly at each series' highest data point. Hover dots are absolutely-positioned `<div>` elements (not SVG `<circle>`) to avoid oval distortion from `preserveAspectRatio="none"`. Fill colors read from CSS variables via `getComputedStyle` (not inline `var()` — WebKit SVG doesn't resolve CSS vars in stop-color). Listens for `sem-colors-changed` custom event to re-read colors when the user changes the semantic color settings; `colorRev` is included in `animKey` to force SVG remount. **Touch interaction**: accepts `gestureMode?: MutableRefObject<GestureMode>` from HomeHero. Touch is handled entirely via native `addEventListener` (not React JSX handlers) registered in a `useEffect` with empty deps — this gives `{ passive: false }` on `touchmove` so `e.preventDefault()` works during scrub. A 300ms long-press timer activates scrub mode: if the finger moves >8px before it fires the timer is cancelled (becomes a swipe handled by HomeHero instead); when it fires `gestureMode` is set to `'scrubbing'` and `haptic('tap')` fires. The container has `select-none` + `style={{ WebkitUserSelect: 'none', WebkitTouchCallout: 'none' }}` to suppress text selection and the iOS magnifier on long press. Mouse hover (`handleMouseMove`/`handleMouseLeave`) is still React JSX and works independently.
...
**Semantic color classes (`text-emerald`, `text-gold`)** map to `var(--sem-income)` / `var(--sem-expense)` via `globals.css`. When the user sets a *gradient* accent color in Settings → Appearances, the CSS rule adds `-webkit-text-fill-color: transparent` to those elements so the gradient background clips to the text shape. This transparent fill **inherits** into child elements — any component that renders text inside `overflow: hidden` containers (like `SlotNumber`'s drum columns) will have invisible digits unless it resets `WebkitTextFillColor: 'currentColor'` on its inner span. `SlotNumber` already does this. If you create a new animated/scroll-clipped text component and wrap it with `text-emerald` or `text-gold`, add `WebkitTextFillColor: 'currentColor'` to its innermost text container.
...
### Touch gesture coordination (`gestureMode` ref pattern)

When a parent component handles swipe-to-navigate and a child component handles long-press-to-scrub on the same touch surface, share a `MutableRefObject<'undecided' | 'swiping' | 'scrubbing'>` between them:

- Parent resets it to `'undecided'` on `touchstart`
- Parent sets it to `'swiping'` the moment it recognises a horizontal flick, and bails out if it's already `'scrubbing'`
- Child (via native `addEventListener`) sets it to `'scrubbing'` when the long-press timer fires (checking first that it's still `'undecided'`)
- Child's `touchend` resets it to `'undecided'`

The child must use **native event listeners** (not React JSX handlers) registered with `{ passive: false }` so `e.preventDefault()` works during scrub. Keep `points` / `onHover` / similar data in refs updated via a separate `useEffect` so the main effect can run with empty deps. Any component with long-press must add `select-none` + `style={{ WebkitUserSelect: 'none', WebkitTouchCallout: 'none' }}` to suppress iOS text selection and the magnifier callout.
...
- `localToday()` — returns today as `YYYY-MM-DD` in `America/Los_Angeles`. **Always use this for any date written to the DB.** Never use `new Date().toISOString().slice(0, 10)` — that's UTC and will produce the wrong date for users in negative-offset timezones. **Exception — calendar `isPast` comparisons**: `todayStr` (used to tint past event pills) must be computed from the browser's local `new Date()`, not `localToday()`. Using `localToday()` here causes a timezone mismatch where events advanced to "today" by LA time appear as "past" in the browser. Pattern: `const _now = new Date(); const todayStr = \`${_now.getFullYear()}-${String(_now.getMonth()+1).padStart(2,'0')}-${String(_now.getDate()).padStart(2,'0')}\``
...
- **215px sidebar** (`background: '#1D2026'`, `borderRight: 1px solid var(--color-grid-border)`, `overflowY: 'auto'`): extends full height to the top of the screen (no separate top bar). Top section contains the List/Month toggle and a settings gear button, with `paddingTop: 'calc(env(safe-area-inset-top, 0px) + 12px)'`. Below that: mini month navigator (`sidebarYear`/`sidebarMonth` state — syncs automatically as the user scrolls the main grid via `document.elementFromPoint()`), per-type legend toggles (`hiddenTypes: Set<EventType>` local state — clicking hides that type from the grid only), Today button scrolls main grid to today, Add Calendar button → settings sheet.
  - **Per-type color customization** (`prefs.typeColors: Record<string, string>`, persisted to `profiles.calendar_prefs` in DB via `CalPrefs`): each legend row has a **rounded-square** colored dot (`borderRadius: 3`, not a circle). Clicking the dot opens a **native `<input type="color">`** via an invisible overlay input; `colorInputRefs = useRef(new Map<string, HTMLInputElement>())` maps each type/calendar ID to its hidden input. Context menu "Change Color" calls `colorInputRefs.current.get(id)?.click()`. A ✕ reset button appears when a custom color is active. Overrides affect Notion grid rendering and are also saved to settings.
  - `notionColor(ev: CalEvent) => string` — inline helper: for Google events: `ev.color ?? prefs.googleCalendarColors?.[calendarId] ?? DOT_COLOR['google']`; for income/sub: `ev.color ?? prefs.typeColors?.[ev.type] ?? DOT_COLOR[ev.type]`. Used for all pill/dot colors inside Notion grid cells.
...
- **Drag-and-drop (large screen, Google events only)**: pills and span bars with `ev.id` are `draggable`. `dragState: { ev, originDate } | null` tracks the in-flight drag; `dragOverDate: string | null` (updated via a ref to avoid excessive re-renders on `onDragOver`) highlights the drop target cell with a gold border overlay. On drop, `handleEventDrop` optimistically updates `googleEvMap` and calls `updateCalEvent`. Dragged pill fades to 40% `opacity` via `isBeingDragged` check. `dragOverRef = useRef<string | null>` prevents state updates when the hovered date hasn't changed. **Critical**: always compute `isBeingDragged` as `!!dragState && !!ev.id && dragState.ev.id === ev.id` — income/sub events have no `id` (`undefined`), so without the `!!ev.id` guard, `undefined === undefined` evaluates `true` and permanently fades all finance pills to 40% opacity.
...
**`CalendarSettingsSheet`** (`CalPrefs`) includes `googleCalendarColors?: Record<string, string>` (calendarId → hex override) and `typeColors?: Record<string, string>` ('income'|'sub' → hex override). Both are saved to `profiles.calendar_prefs` on Done. Color dot buttons in both the Google Calendars list and the Income/Subscriptions list set `swatchFor: { key: string; kind: 'google' | 'finance'; top: number; left: number } | null` state. The floating swatch picker (`zIndex: 80`) renders `COLOR_PALETTE` as 22×22 squares in a 4-column grid; selected swatch gets white checkmark + 2px white border. On pick, `kind === 'finance'` writes to `local.typeColors[key]`; `kind === 'google'` writes to `local.googleCalendarColors[key]`. A ✕ reset button appears when `isCustom` for either kind.
