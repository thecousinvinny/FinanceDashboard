...
- `home/SparkChart.tsx` — 14-day daily area chart with three series: `inc` (income), `exp` (non-sub expenses), `sub` (subscription payments). Values are **individual daily amounts** (not cumulative) for the rolling 14-day window ending today. X-axis: up to 7 sparse absolute-positioned landmark labels. `DayPoint { day, label, exp, inc, sub }` — all three fields are daily totals for that date. Gradients use `gradientUnits="userSpaceOnUse"` with per-series `peakY` so opacity 0.95 falls exactly at each series' highest data point. Hover dots are absolutely-positioned `<div>` elements (not SVG `<circle>`) to avoid oval distortion from `preserveAspectRatio="none"`.
