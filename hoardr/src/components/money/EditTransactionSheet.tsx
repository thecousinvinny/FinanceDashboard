...
interface Props {
  ...
  open:    boolean
  onClose: () => void
  onSave:  (id: string, updates: { name: string; amount: number; category: string; date: string }) => void
}

export function EditTransactionSheet({ tx, open, onClose, onSave }: Props) {
  const [amount,   setAmount]   = useState('')
  const [name,     setName]     = useState('')
  const [category, setCategory] = useState<string | null>(null)
  const [date,     setDate]     = useState('')

  // Populate fields when a new tx is opened
  useEffect(() => {
    if (tx) {
  ...
  function handleSave() {
    ...
    onSave(tx.id, { name: name.trim(), amount: parsed, category, date })
    onClose()
  }
